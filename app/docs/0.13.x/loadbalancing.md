...
entities.
  ...
  - `upstream`: a 'virtual hostname' which can be used in a Service `host`
    ...
    from a Service with `url=http://weather.v2.service/some/path`.
...
an Service. Switching target infrastructure only requires a `PATCH` request on an
Service, to change the `upstream` name.
...
# create a Service and Route targeting the Blue upstream
...
$ curl -X PATCH http://kong:8001/services/address-service \
    --data "url=http://address.v2.service/address"
```
